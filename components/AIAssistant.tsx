  ...
  return (
    <>
      ...
      <button 
        ...
        className="fixed bottom-28 right-6 w-14 h-14 bg-gradient-to-tr from-blue-600 to-indigo-600 text-slate-100 rounded-full shadow-2xl flex items-center justify-center z-50 hover:scale-110 active:scale-95 transition-all border-4 border-white/20"
      >
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-6 bg-slate-900/40 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white w-full max-w-lg h-[80vh] sm:h-[600px] sm:rounded-[2.5rem] rounded-t-[2.5rem] shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 border border-slate-100">
            {/* Header */}
            <div className="p-6 bg-gradient-to-r from-blue-600 to-indigo-700 text-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-white/10 rounded-xl backdrop-blur-md text-slate-100">
                  <Bot size={24} />
                </div>
                <div>
              ...
              <button onClick={() => setIsOpen(false)} className="p-2 hover:bg-white/10 rounded-full transition-colors text-slate-100">
                <X size={20} />
              </button>
            </div>
                ...
                <div className="h-full flex flex-col items-center justify-center text-center p-10 opacity-60">
                  ...
                  <div className="mt-4 flex flex-wrap justify-center gap-2">
                    {["Бетон М350 СНиП", "Нормы армирования", "Охрана труда"].map(q => (
                      <button key={q} onClick={() => setInput(q)} className="text-[9px] font-black uppercase tracking-widest px-3 py-2 bg-white border border-slate-100 rounded-lg text-blue-600 hover:bg-blue-50">
                        {q}
                      </button>
                    ))}
                  </div>
              ...
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] p-4 rounded-2xl text-sm font-medium leading-relaxed ${
                    m.role === 'user' 
                      ? 'bg-blue-600 text-slate-100 shadow-lg' 
            ...
            <div className="p-4 bg-white border-t border-slate-100 flex gap-2">
              <input 
                ...
                className="flex-1 bg-slate-50 px-5 py-4 rounded-2xl text-sm font-bold text-slate-900 placeholder:text-slate-400 border border-slate-100 outline-none focus:ring-4 focus:ring-blue-50 transition-all"
              />
              <button 
                onClick={handleSend}
                disabled={isLoading}
                className="bg-blue-600 text-slate-100 p-4 rounded-2xl shadow-xl shadow-blue-100 active:scale-95 disabled:opacity-50 transition-all"
              >
                <Send size={24} />
              </button>
            </div>
